...
{
  NS_LOG_FUNCTION (this);
  m_zzTs = m_synchronizer->GetCurrentRealtime();
}
    ...
    if (m_synchronizationMode == SYNC_HARD_LIMIT)
      {
        ...
        if (tsJitter > static_cast<uint64_t> (m_hardLimit.GetTimeStep ()))
          {
            uint64_t ts_zz_jitter = (tsFinal >= m_zzTs) ?
                               tsFinal - m_zzTs : m_zzTs - tsFinal;
            if (ts_zz_jitter > static_cast<uint64_t> (m_hardLimit.GetTimeStep ()))
              {
                // print warning but do not print another 
                m_zzTs = m_synchronizer->GetCurrentRealtime ();
                std::cerr << "Hard real-time limit exceeded" << std::endl;
              }
//zz            NS_FATAL_ERROR ("RealtimeSimulatorImpl::ProcessOneEvent (): "
//zz                            "Hard real-time limit exceeded (jitter = " << tsJitter << ")");
          }
      }
  }
